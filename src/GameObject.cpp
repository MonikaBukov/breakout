...
void GameObject::bounce(GameObject& sprite1)
{
  if (is_collide(sprite1))
  {
    if (top_collide(sprite1) && (sprite->getPosition().y > sprite1.getMidPointY()))
    {
      direction.x = getMidPointX() - sprite1.getMidPointX();
      direction.y = 100;
      direction.normalise();
    }
    else if (bottom_collide(sprite1) && (sprite->getGlobalBounds().height + sprite->getPosition().y < sprite1.getMidPointY()))
    {
      direction.x = getMidPointX() - sprite1.getMidPointX();
      direction.y = - 100;
      direction.normalise();
    }
    else if (left_collide(sprite1))
    {
      direction.x = - direction.x;
    }
    else if (right_collide(sprite1))
    {
      direction.x = - direction.x;
    }
  }
